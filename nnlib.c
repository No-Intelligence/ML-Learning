...
void add_conv_layer (neural_network_t *nn, int in_height, int in_width, int in_channel, int filter_height, int filter_width, int n_filers, int filter_stride, int n_padding) {
    ...
    nn->layers[nn->n_layers - 1].output_size = n_filers * out_height * out_width;
    nn->layers[nn->n_layers - 1].output = calloc(n_filers * out_height * out_width, sizeof(float));
    ...
    nn->layers[nn->n_layers - 1].data.conv.n_filters = n_filers;
    nn->layers[nn->n_layers - 1].data.conv.filter_stride = filter_stride;
    nn->layers[nn->n_layers - 1].data.conv.n_padding = n_padding;
    nn->layers[nn->n_layers - 1].data.conv.filter = calloc(in_channel * n_filers * filter_height * filter_width, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.bias = calloc(n_filers, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.m_filter = calloc(in_channel * n_filers * filter_height * filter_width, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.v_filter = calloc(in_channel * n_filers * filter_height * filter_width, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.grad_filter = calloc(in_channel * n_filers * filter_height * filter_width, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.total_grad_filter = calloc(in_channel * n_filers * filter_height * filter_width, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.m_bias = calloc(n_filers, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.v_bias = calloc(n_filers, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.grad_bias = calloc(n_filers, sizeof(float));
    nn->layers[nn->n_layers - 1].data.conv.total_grad_bias = calloc(n_filers, sizeof(float));
}
...
void forward_convolution (float *input, float *filter, float *output, int n_input_height, int n_input_width, int n_input_channel, int filter_height, int filter_width, int n_filters, int stride, float *bias) {
    //standby
    int n_output_height = n_input_height - filter_height + 1;
    int n_output_width = n_input_width - filter_width + 1;

    //zero fill
    memset(output, 0.0f, n_filters * n_output_height * n_output_width * sizeof(float));
